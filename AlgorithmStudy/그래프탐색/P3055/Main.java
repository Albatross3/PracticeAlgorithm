...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0; i<R; i++){
            ...
            for(int j=0; j<C; j++){
                map[i][j]=s.charAt(j);
                if(map[i][j]=='S') P=new Position(i,j,'S');
                if(map[i][j]=='*') queue.add(new Position(i,j,'*'));
            }
        }
        queue.add(P);
                ...
                if(newX>=0 && newX<R && newY>=0 && newY<C){
                    if(p.getType()=='.' || p.getType()=='S'){
                        if((map[newX][newY]=='.'|| map[newX][newY]=='D') && dp[newX][newY]==0){
                            ...
                            //          6. 큐에 넣기
                            queue.add(new Position(newX,newY,map[newX][newY]));
                        }
                    }
...
class Position{
    int i,j;
    char type;
    public Position(int i,int j, char type){
        this.i=i;
        this.j=j;
        this.type=type;
    }
